...
fn instruction_drw() {
    { // Draw at 0,0
        let mut cpu = CPU::new();
        cpu.set_memory(&[
            0xD0, 0x06, // DRW, V0, V0, n
            0xFF, 0xFF, // Sprite data
            0x00, 0x00, // Sprite data
            0xFF, 0xFF, // Sprite data
        ]);
        cpu.set_i(cpu.get_pc() + 2);
        cpu.tick();
        assert_eq!(cpu.get_display()[0..48], [
    ...
    { // Draw at 60,0 - Horizontal wrap
        let mut cpu = CPU::new();
        cpu.set_memory(&[
            0xD0, 0x16, // DRW, V0, V1, n
            0xFF, 0xFF, // Sprite data
            0x00, 0x00, // Sprite data
            0xFF, 0xFF, // Sprite data
        ]);
        cpu.set_registers(&[60, 0]);
        cpu.set_i(cpu.get_pc() + 2);
        cpu.tick();
        assert_eq!(cpu.get_display()[0..48], [
    ...
    { // Draw at 60,28 - Vertical + horizontal wrap
        let mut cpu = CPU::new();
        cpu.set_memory(&[
            0xD0, 0x16, // DRW, V0, V1, n
            0xFF, 0xFF, // Sprite data
    ...
    { // Overwrite
        let mut cpu = CPU::new();
        cpu.set_memory(&[
            0xD0, 0x12, // DRW, V0, V1, n
            0xD0, 0x12, // DRW, V0, V1, n
            0xD0, 0x12, // DRW, V0, V1, n
            0xFF, 0x00, // Sprite data
            0x00, 0xFF, // Sprite data
            0xF0, 0xF0, // Sprite data
        ]);
        let sprite_index = cpu.get_pc() + 6;
        cpu.set_registers(&[0, 0]);
        cpu.set_i(sprite_index);
        cpu.tick();
        assert_eq!(cpu.get_display()[0..16], [
            0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ]);
        assert_eq!(cpu.get_vf(), 0);
        cpu.set_i(sprite_index + 2);
        cpu.tick();
        assert_eq!(cpu.get_display()[0..16], [
            0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
